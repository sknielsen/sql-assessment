...
SELECT m.year, m.name, model_id, a.name as name FROM models AS m LEFT JOIN awards as a ON model_id = winner_id WHERE m.year >= 1964;
...
SELECT brands.name, founded FROM brands LEFT JOIN models ON models.brand_id = brands.brand_id WHERE models.name is NULL;
...
SELECT brands.brand_id FROM brands JOIN models ON brands.brand_id = models.brand_id GROUP BY brands.brand_id HAVING COUNT(*) > 5;
